...
import { MentorEventCard } from "@/components/MentorEventCard";
import { Calendar, Filter, Search, Plus } from "lucide-react";
import { useState, useEffect } from "react";
import { Button } from "@/components/ui/button";

export const Route = createFileRoute("/mentor/community/events/")({
  head: () => ({
    meta: [
      { title: "Events — Sandiwa Mentor" },
      { name: "description", content: "Manage and host community events." },
    ],
  }),
...
function EventsListPage() {
  const [filter, setFilter] = useState<"all" | "upcoming" | "free" | "virtual" | "my">("upcoming");
  ...
  useEffect(() => {
    ...
    const storedMentorId = localStorage.getItem("mentorId") || sessionStorage.getItem("mentorId") || "";
  ...
  // Filter by "My Events" - events created by this mentor
  if (filter === "my") {
      ...
      <MentorDashboardLayout>
        <section className="bg-gradient-to-br from-cream to-white py-16">
          <div className="mx-auto max-w-7xl px-4 text-center">
            <p className="text-sm font-semibold uppercase tracking-wider text-gold">Host & Join</p>
            <h1 className="mt-2 font-serif text-5xl font-bold text-navy sm:text-6xl">
              Community Events
            </h1>
            <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">
              Host workshops, meetups, and cultural celebrations with fellow culture enthusiasts.
            </p>
        ...
        <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {filteredEvents.map((event) => (
              <Link key={event.id} to="/mentor/community/events/$eventId" params={{ eventId: event.id }}>
          ...
          {filteredEvents.length === 0 && (
            <div className="py-20 text-center">
              ...
              <p className="mt-4 text-muted-foreground">
                {filter === "my" ? "You haven't created any events yet." : "No events match your search or filter."}
              </p>
              {filter === "my" && (
                <Link to="/mentor/community/events/create">
