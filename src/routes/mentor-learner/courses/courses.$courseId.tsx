...
export const Route = createFileRoute("/mentor-learner/courses/courses/$courseId")({
  loader: ({ params }) => {
  ...
  notFoundComponent: () => (
    <MentorDashboardLayout>
      <div className="mx-auto max-w-2xl py-24 text-center">
        ...
        <Link to="/mentor-learner/courses/courses" className="mt-4 inline-block text-gold hover:underline">
          Back 
        </Link>
      </div>
    </MentorDashboardLayout>
...
function CourseDetail() {
  ...
  const handleEnrollNow = () => {
    ...
    if (alreadyEnrolled) {
      navigate({ to: "/mentor-learner/learn/$courseId", params: { courseId: course.id } });
      return;
    }
    
    cart.add(course);
    navigate({ to: "/mentor-learner/checkout" });
  };
  ...
  const handleGoBack = () => {
    ...
    if (document.referrer && document.referrer.includes(window.location.origin)) {
      ...
      navigate({ to: "/mentor-learner/courses/courses" });
    }
  };
