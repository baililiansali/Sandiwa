...
// export const Route = createFileRoute("/learner/certificates/")({
...
//   useEffect(() => {
//     const enrolled = JSON.parse(localStorage.getItem("sandiwa.enrolled") || "[]");
//     const completed = enrolled.filter((c: any) => c.progress === 100);
    
//     const certs = completed.map((c: any) => {
...
//                           <span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> Completed: {new Date(cert.completedAt).toLocaleDateString()}</span>
//                           <span className="flex items-center gap-1"><CheckCircle className="h-3 w-3 text-green-600" /> Verified</span>
//                         </div>
//                       </div>
//                     </div>
...
// src/routes/learner/certificates/index.tsx
import { createFileRoute, Link } from "@tanstack/react-router";
import { SiteLayout } from "@/components/SiteLayout";
import { AuthGuard } from "@/components/AuthGuard";
import { Award, Download, Calendar, CheckCircle } from "lucide-react";
