...
export function EventCard({ event, compact = false }: EventCardProps) {
  ...
  if (compact) {
    return (
      <Link
        to="/learner/community/events/$eventId"
      ...
      >
        ...
        <span className="text-sm text-gold group-hover:underline">View Details →</span>
      </Link>
    );
  }
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between pt-4 border-t border-border">
          ...
          <span className="text-sm text-gold group-hover:underline">Learn More →</span>
        </div>
      </div>
    </Link>
  );
